...
$(function () {
    ...
    function randomPosition(boundary, distance, prohibited = []) {
        ...
        } while (onObject([0, 0], newPosition, distance) || prohibited.some(position => (position[0] === newPosition[0] && position[1] === newPosition[1])));
        return newPosition;
    }
    function logMessage(message, fromUser = false) {
        ...
        if (typeof scrollHeight === 'number')
            $('#command-history-div').scrollTop(scrollHeight);
    }
    ...
    ['n', 'e', 's', 'w'].forEach(direction => {
        $(`#bttn-move-${direction}`).on('click', () => {
            // TODO: Select element with jQuery
            $('#command-input').val(`move ${direction}`);
            // const commandInput = document.getElementById('command-input') as HTMLInputElement;
            // commandInput.value = `move ${direction}`;
